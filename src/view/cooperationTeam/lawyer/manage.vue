    ...
    <div class="customer-manage have-drawer-style relationship">
        <div class="card-style">
            ...
            <Row class="header-common-right-box">
                <Col style="float:left;">
                    <Dropdown @on-click="headerOption" v-permission="['cooperation:lawyerteam:manage:manage-']">
            ...
            <Row style="border:1px solid #dcdee2;border-bottom:0;">
                <checkbox-table :columns="columns" :data="data" ref="checkboxTable"
                    ...
                    <template slot-scope="{ row, index }" slot="more-actions">
                        <Dropdown @on-click="option(row,$event)" v-permission="['cooperation:lawyerteam:manage:manage-']">
        ...
        <Modal
            ...
            <Form ref="formValidate" :model="formValidate" :rules="ruleValidate">
                <Row class="modal-tips">请问是否确认禁用该律师的账号信息？</Row>
        ...
        <Modal
            ...
            :width="360"
            :footer-hide="true">
            <Row class="modal-tips">请问是否确认启用该律师的账号申请？</Row>                                                                                                                                                                                      
        ...
        <Drawer 
            ...
            <div slot="header" class="drawer-title">
                律师详情
                <Dropdown transfer @on-click="option(lawyerDetail,$event)" class="edit-icon" v-permission="['cooperation:lawyerteam:manage:manage-']">
                    <a href="javascript:void(0)">
                        <Icon type="ios-more" size="20"/>
                    </a>
                    <DropdownMenu slot="list">
                        ...
                        <DropdownItem v-if="lawyerDetail.state=='R'" name="reject-reason">查看审核驳回原因</DropdownItem>
                        <DropdownItem v-if="lawyerDetail&&(!lawyerDetail.status)" name="disable-reason">查看禁止原因</DropdownItem>
                        <DropdownItem v-if="lawyerDetail&&lawyerDetail.status&&lawyerDetail.state!='P'" name="reject">禁用</DropdownItem>
                        <DropdownItem v-if="lawyerDetail&&(!lawyerDetail.status)&&lawyerDetail.state!='P'" name="agree">启用</DropdownItem>
                    </DropdownMenu>
                </Dropdown>
            </div>
            <Row>
...
export default {
        ...
        return {
            ...
            columns:[
                ...
                },
                {
                    ...
                    key:"auditTime",
                    render:(h,params)=>{
                        return h('span',params.row.auditTime&&params.row.auditTime.slice(0,-9));
                    }
                },
                {
                    title:"审核状态",
    ...
    methods: {
        ...
        exportExcel(type){
            let url = '/base/lawyer-teams/export';
            let ajaxData = {
                state:this.auditStatus,       
                province:this.province,    
                city:this.city, 
        ...
        enableSubmit(){
            let url = '/base/lawyer-team/enable';
            let ajaxData = {
        ...
        handleSubmit (name) {
            let url = '/base/lawyer-team/disable';
            let ajaxData = {
