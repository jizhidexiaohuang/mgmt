                        ...
                        <div class="case-package-item" v-for="(item,index) in data" :key="index">
                            ...
                            <div class="package-content">
                                <Row class="package-row">
                                    <span class="left-label">案件包区域:</span>
                                    <span>{{getLocationStr(item.caseSaleBagLocationList)}}</span>
                                </Row>
                                <Row class="package-row">
                                    <Col span="12">
                                        <span class="left-label">购买团队:</span>
                                        <span v-if="item.purchaseTeamNum==0">{{item.purchaseTeamNum}}</span>
                                        <span v-else><a href="javascript:void(0);" @click="routerToOrder(item.id)">{{item.purchaseTeamNum}}</a></span>
                                    </Col>
                                    <Col span="12">
                                ...
                                <Row class="package-row">
                                    <Col span="12">
                                        <span class="left-label">已预约:</span>
                                        <span>{{item.reservedNum || 0}}件</span>
                                    </Col>
                                    <Col span="12">
                                        <span class="left-label">已出售:</span>
                                        <span>{{item.soldNum || 0}}件</span>
                                    </Col>
                                </Row>
                                <Row class="package-row">
                                    <Col span="15">
                                    ...
                                    <Col span="9" class="error-time">
                                        <Icon type="md-alarm" />
                                        <count-down :key="item.publishState" :time="item.protectSeconds" style="vertical-align: middle;"></count-down>
                                    </Col>
                                </Row>
                            </div>
                        </div>
...
export default {
    data () {
        return {
            ...
            columns:[
                ...
                {
                    title:"区域",
                    key:"caseSaleBagLocationList",
                    render:(h,params)=>{
                        let str = this.getLocationStr(params.row.caseSaleBagLocationList || []);
                        return h('span',{
                            class:'textover',
                            attrs:{ title:str }
                        },str)
                ...
                },
                {
                    ...
                    render:(h,params)=>{
                        return h("countDown",{
                            class:'textover',
                            props:{
                                time:params.row.protectSeconds,
                            }
                        },"")
                    }
                },
                {
                    ...
                    key:"caseNum",
                    render:(h,params)=>{
                        //forSaleNum待出售 reservedNum已预约 soldNum已出售 returnNum已退回 caseNum总案件数
                        let iconDom = h("span","/");
                        let forSaleNumDom = h("span",{ style:{ color:styles.warningColor } },`${params.row.forSaleNum || 0}`);
                        let reservedNumDom = h("span",{ style:{ color:styles.successColor } },`${params.row.reservedNum || 0}`);
                        let soldNumDom = h("span",{ style:{ color:styles.infoColor } },`${params.row.soldNum || 0}`);
                        let caseNumDom = h("span",`${params.row.caseNum || 0}`);
                        let contentDom = h("div",{slot:"content"},[
                            h("p",{ style:{ color:styles.warningColor,marginBottom:"5px" } },`待出售：${params.row.forSaleNum || 0}件`),
                            h("p",{ style:{ color:styles.successColor,marginBottom:"5px" } },`已预约：${params.row.reservedNum || 0}件`),
                            h("p",{ style:{ color:styles.infoColor,marginBottom:"5px" } },`已出售：${params.row.soldNum || 0}件`),
                            h("p",`总案件数：${params.row.caseNum || 0}件`)
                        ])
                        let toolTip = h("Tooltip",{
                            props:{ theme:"light" }
                        },[forSaleNumDom,iconDom,reservedNumDom,iconDom,soldNumDom,iconDom,caseNumDom,contentDom])
                        return toolTip
                    }
                },
                {
                    ...
                    key:'prepaidFee',
                    render:(h,params)=>{
                        let str = this.common.formateNumber(params.row.prepaidFee || 0);
                        return h("span",str)
                    }
                },
                {
                    ...
                    key:"paidBalanceFee",
                    render:(h,params)=>{
                        let str = this.common.formateNumber(params.row.paidBalanceFee || 0);
                        return h("span",str)
                    }
                },
                {
                ...
                },
                {
                    ...
                    render:(h,params)=>{
                        let dom = h("a",{
                            ...
                            on:{
                                "click":()=>{
                                    this.routerToOrder(params.row.id);
                                }
                            }
                        },params.row.purchaseTeamNum);
                        if(params.row.purchaseTeamNum==0){
                            return h("span",params.row.purchaseTeamNum)
                        }else{
                            return dom
                        }
                    }
    ...
    methods: {
        getData(){
            let url = '/settlement/trade/sale/bag/public';
            let ajaxData = {
