<template>
    <div>
        <Form :model="formData" ref="formData" :rules="ruleValidate" :label-width="100">
            ...
            <Row class="detail-form-block">
                ...
                <Row class="detail-btn-box">
                    <Button type="primary" style="margin-right:10px;" :disabled="isSubmit" @click="confirmCtrl=true">保存</Button>
        ...
        <Modal
            ...
            :width="360"
            :footer-hide="true">
            <Row class="modal-tips">请问是否保存本次修改？</Row>
    ...
    methods: {
        submitNewSettlement(name){
            ...
            this.$refs[name].validate((valid) => {
                if(valid){
                    ajaxData.configOnline.id = this.configDetail.configOnline.id;
                    ajaxData.configOffline.id = this.configDetail.configOnline.id;
                    this.isSubmit = true;
        ...
        getCustomerDetail(data){
            for(let key in this.formData){
                if(key==='gradeConfigList'){
                    this.formData.gradeConfigList = data.configOffline.gradeConfigList.map(item=>{
                        return {
                            grade:item.grade,
                            id:item.id,
