    ...
    <div class="have-drawer-style">
        <div class="card-style">
            ...
            <Row style="border:1px solid #dcdee2;border-bottom:0;">
                <Table :columns="columns" :data="data" :loading="tableLoading">
                    <template slot-scope="{ row, index }" slot="action" v-permission="['agreement:investigator:audit-']">
                        <Dropdown @on-click="option(row.id,$event)">
                            <a href="javascript:void(0)">
                                <Icon type="ios-more" size="20"/>
                            </a>
                            <DropdownMenu slot="list">
                                <DropdownItem name="audit" v-show="row.state=='P'">审核</DropdownItem>
        ...
        <Drawer 
            ...
            <div slot="header" class="drawer-title">
                ...
                <Dropdown transfer @on-click="option(investAgreementDetail.id,$event)" v-if="isInfo" class="edit-icon" v-permission="['agreement:investigator:audit-']">
                    <a href="javascript:void(0)">
                        <Icon type="ios-more" size="20"/>
                    </a>
                    <DropdownMenu slot="list">
                        <DropdownItem name="audit" v-if="investAgreementDetail.state=='P'">审核</DropdownItem>
                        <DropdownItem name="reject" v-if="investAgreementDetail.state=='P'">驳回</DropdownItem>
                    </DropdownMenu>
                </Dropdown>
            </div>
            <investigator-agreement-form v-show="!isInfo && addDrawerCtrl" :iscopy="iscopy" :detail="investAgreementDetail" @changeEditCtrl="changeCtrl"></investigator-agreement-form>
            <investigator-agreement-info v-show="isInfo && addDrawerCtrl" :detail="investAgreementDetail"></investigator-agreement-info>
        </Drawer>
...
export default {
    ...
    methods: {
        ...
        getData(){
            let url = '/settlement/agreement-price-investigators/pending';
            let ajaxData = {
        ...
        delSubmit(){
            ...
            }else if(this.type==='audit' || this.type=='reject'){    // 作废 审核 驳回
                url += '/'+this.type;
