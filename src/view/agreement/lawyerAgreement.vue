    ...
    <div class="have-drawer-style">
        <div class="card-style">
            <Row class="header-common-right-box">
                <Col style="float:left;">
                    <Button @click="openAddDrawer" v-if="false" type="primary">新增律师协议</Button>
                </Col>
                <!-- 筛选项 -->
            ...
            <Row style="border:1px solid #dcdee2;border-bottom:0;">
                <Table :columns="columns" :data="data" :loading="tableLoading">
                    <template slot-scope="{ row, index }" slot="action">
                        <Dropdown @on-click="option(row.id,$event,row.name)" v-permission="['agreement:lawyer:all:manage-']">
                            <a href="javascript:void(0)">
                                <Icon type="ios-more" size="20"/>
                            </a>
                            <DropdownMenu slot="list">
                                <DropdownItem name="edit" v-show="getState(row.state,row.missionId)">编辑</DropdownItem>
                                <DropdownItem name="release" v-permission="['agreement:lawyer:audit-']" v-show="row.state=='D'||row.state=='P'">发布</DropdownItem>
                                <DropdownItem name="release-P" v-permission="['agreement:lawyer:audit-',false]" v-show="row.state=='D'">发布</DropdownItem>
                                ...
                                <DropdownItem name="reject" v-show="row.state=='P'">驳回</DropdownItem>
        ...
        <Drawer 
            ...
            <div slot="header" class="drawer-title">
                ...
                <Dropdown transfer @on-click="option(agreementDetail.id,$event,agreementDetail.name)" v-if="isInfo" class="edit-icon" v-permission="['agreement:lawyer:all:manage-']">
                    <a href="javascript:void(0)">
                        <Icon type="ios-more" size="20"/>
                    </a>
                    <DropdownMenu slot="list">
                        <DropdownItem name="edit" v-show="getState(agreementDetail.state,agreementDetail.missionId)">编辑</DropdownItem>
                        <DropdownItem name="release" v-permission="['agreement:lawyer:audit-']" v-show="agreementDetail&&(agreementDetail.state=='D'||agreementDetail.state=='P')">发布</DropdownItem>
                        <DropdownItem name="release-P" v-permission="['agreement:lawyer:audit-',false]" v-show="agreementDetail&&agreementDetail.state=='D'">发布</DropdownItem>
                        <DropdownItem name="delete" v-show="agreementDetail&&agreementDetail.state=='D'">删除</DropdownItem>
                        <DropdownItem name="cancel" v-show="agreementDetail&&(agreementDetail.state=='E'&& !agreementDetail.missionId) || agreementDetail.state=='R'">作废</DropdownItem>
                        <DropdownItem name="reject" v-show="agreementDetail&&agreementDetail.state=='P'">驳回</DropdownItem>
                        <DropdownItem name="copy">复制</DropdownItem>
                    </DropdownMenu>
                </Dropdown>
            </div>
            <lawyer-agreement-form v-if="!isInfo && addDrawerCtrl" :iscopy="iscopy" :agreementDetail="agreementDetail" @changeEditCtrl="changeCtrl"></lawyer-agreement-form>
            <lawyer-agreement-info v-if="isInfo && addDrawerCtrl" :agreementDetail="agreementDetail"></lawyer-agreement-info>
        </Drawer>
...
export default {
    ...
    methods: {
        ...
        getData(){
            let url = '/settlement/agreement-price-lawyers';
            let ajaxData = {
        ...
        delSubmit(){
            ...
            }else if(this.modalType=="cancel" || this.modalType=="release" || this.modalType=="release-P" || this.modalType=="audit" || this.modalType=="reject"){    
                // 作废 | 发布-生效 | 发布-待审核 | 审核通过 | 审核驳回
                if(this.modalType==='release-P'){
                    url += '/submit';
