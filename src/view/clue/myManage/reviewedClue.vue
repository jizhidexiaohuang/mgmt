<template>
    <div class="my-clue-result have-drawer-style">
            ...
            <ul class="list">
                ...
                <li v-for="(item,index) in data" :key="index">
                    ...
                    <div :class="{'result-title':true,'conflict-title':(item.conflictCategory==1 || item.conflictCategory==3)}">
                        <Checkbox :value="idList.indexOf(item.id)>=0?true:false" @on-change="checkOption(item)"></Checkbox>
                        ...
                        <span class="result-title-option">
                            <Dropdown @on-click="listOption(item.id,$event)" transfer>
        ...
        <Modal
            ...
            :width="450"
            :footer-hide="true">
            <Row v-if="!rejectCtrl" :style="{color:mainColor.errorColor}">注:（需要进行测购的线索，测购审核通过才可以转案件）</Row>
            <Form ref="formValidate" :key="type" :model="formValidate" :label-width="80">
                <FormItem label="驳回原因" prop="reasonId" v-if="rejectCtrl"
                    :rules="[{required: true,type:'number', message: '驳回原因不能为空', trigger: 'blur'}]">
    ...
    methods: {
        storeName(item){
            switch (true){
                ...
                case (item.tortType==='E_EC'&&(item.channelType==='TT'||item.channelType=='PD')):
                    ...
                    if(this.rejectCtrl){
                        ...
                        if(this.batchCtrl){
                            ...
                            this.idObjList.forEach((item,index)=>{
                                if(!item.isTestPurchase){
                                    ajaxData.clueIds.push(item.id);
                                }else{
                                    testPurchaseIds.push(item.id);
                                }
                            })
                        }
                        ...
                        if(testPurchaseIds.length!=0){
                            this.firstPass(testPurchaseIds,this.formValidate.examineRemark);
                        }
                        if(ajaxData.clueIds.length==0){
                            this.examineCtrl = false;
                            return
                        }
                        this.trunCaseLoading = true;
        ...
        getData(){
            let url = '/business/clues/my/pending';
            let ajaxData = {
        ...
        //选中多选框
        checkOption(item){
            let idIndex = this.idList.indexOf(item.id)
            if (idIndex >= 0) {//如果已经包含就去除
                this.idList.splice(idIndex, 1);
                this.idObjList.splice(idIndex,1);
            } else {//如果没有包含就添加
            ...
            if(this.isCheckedAll){
                if(this.idList.length>0){
                    for(var i=0;i<this.pageIdList.length;i++){
                        for(var j=0;j<this.idList.length;j++){
                            if(this.pageIdList[i]==this.idList[j]){
                                ...
                                this.idObjList.push(this.pageIdObjList[i]);
                            }
                        }
                    }
                }else{
                    this.pageIdList.forEach((item,index)=>{
                        ...
                        this.idObjList.push(this.pageIdObjList[index]);
                    })
                }
            }else{
                for(var i=0;i<this.pageIdList.length;i++){
