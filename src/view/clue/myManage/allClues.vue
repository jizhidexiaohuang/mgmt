<template>
    <div class="my-clue-result have-drawer-style">
        <div class="card-style">
            ...
            <Row class="header-common-right-box">
                <Col style="float:left;margin-left:10px;">
                    ...
                    <Dropdown @on-click="headerOption">
                        ...
                        <DropdownMenu slot="list">
                            <!-- <DropdownItem :disabled="idList.length==0" name="agree" v-permission="['clue:audit-']">批量通过/转案件</DropdownItem> -->
                            <!-- <DropdownItem :disabled="idList.length==0" name="reject" v-permission="['clue:audit-']">批量驳回</DropdownItem> -->
            ...
            <!-- 表格 -->
            <ul v-show="!caseInfoCtrl" class="list">
                <Spin fix v-if="tableLoading"></Spin>
                <li v-for="(item,index) in data" :key="index">
                    <!-- 0 无冲突无关联  ||  1 有冲突  ||  2 有关联  ||  3 有冲突有关联 -->
                    <div :class="{'result-title':true,'conflict-title':(item.conflictCategory==1 || item.conflictCategory==3)}">
                        ...
                        <span class="result-name" @click="getClueDetail(item.id,true)">
                        ...
                        <span class="result-title-option">
                            <Dropdown @on-click="listOption(item.id,$event)">
                                ...
                                <DropdownMenu slot="list">
                                    ...
                                    <DropdownItem name="edit" v-if="item.state=='P'||item.state=='R'">编辑</DropdownItem>
                                    <DropdownItem name="agree" v-if="item.state=='P'&&!item.isTestPurchase">通过</DropdownItem>
                                    <DropdownItem name="firstAgree" v-if="item.state=='P'&&item.isTestPurchase">通过</DropdownItem>
                                    <DropdownItem name="agree" v-if="(item.state=='E'&&!item.isTestPurchase) || (item.state=='E'&&item.isTestPurchase&&item.testPurchaseState=='E')">转案件</DropdownItem>
                                    <DropdownItem name="reject" v-if="item.state=='P'">驳回</DropdownItem>
                                    <DropdownItem name="push" v-if="!item.custState">推送</DropdownItem>
                                    <DropdownItem name="conflict" v-if="item.conflictCategory==1 || item.conflictCategory==3">查看冲突</DropdownItem>
                                    <DropdownItem name="relate" v-if="item.conflictCategory==2 || item.conflictCategory==3">查看关联</DropdownItem>
                                </DropdownMenu>
                            </Dropdown>
                        </span>
                    </div>
            ...
            <Row v-show="!caseInfoCtrl" style="margin-top:30px;text-align:center;">
        ...
        <Drawer 
            ...
            :width="isInfo?800:600"
            :styles="{padding:'0px'}"
            :mask-style="{background:'rgba(0,0,0,0)'}">
            <div slot="header" class="drawer-title">
                <span>{{clueDetail.code}}---{{clueDetail.missionName}}</span>
                <Dropdown @on-click="listOption(clueDetail.id,$event)" class="edit-icon" v-if="isInfo">
                    <a href="javascript:void(0)">
                        <Icon type="ios-more" size="20"/>
                    </a>
                    <DropdownMenu slot="list">
                        <DropdownItem name="edit" v-if="clueDetail.state=='P'||clueDetail.state=='R'">编辑</DropdownItem>
                        <DropdownItem name="agree" v-if="clueDetail.state=='P'&&!clueDetail.isTestPurchase">通过</DropdownItem>
                        <DropdownItem name="firstAgree" v-if="clueDetail.state=='P'&&clueDetail.isTestPurchase">通过</DropdownItem>
                        <DropdownItem name="agree" v-if="(clueDetail.state=='E'&&!clueDetail.isTestPurchase) || (clueDetail.state=='E'&&clueDetail.isTestPurchase&&clueDetail.testPurchaseState=='E')">转案件</DropdownItem>
            ...
            <clue-Info v-if="isInfo && clueDetailCtrl" :clueDetail="clueDetail"></clue-Info>
            <clue-form v-if="!isInfo && clueDetailCtrl" :clueDetail="clueDetail" @changeEditCtrl="changeCtrl"></clue-form>
            <test-purchase-form v-if="isInfo&&clueDetail.state!='P'&&clueDetail.isTestPurchase&&clueDetailCtrl" :clueDetail="clueDetail"></test-purchase-form>
        </Drawer>
        ...
        <Modal
            ...
            <Form ref="formValidate" :key="type" :model="formValidate" :label-width="80">
                ...
                <FormItem label="打击方式" prop="caseType" v-if="!rejectCtrl"
                    :rules="[{required: true, message: '打击方式不能为空', trigger: 'blur'}]">
                    <RadioGroup v-model="formValidate.caseType" v-for="(item,index) in caseTypeArr" :key="index">
                        <Radio :label="item.value">{{item.nameZh}}</Radio>
                    </RadioGroup>
                </FormItem>
                <FormItem label="关联案件" prop="caseIdList" v-if="!rejectCtrl&&(!batchCtrl)">
            ...
            <Row style="text-align:center;">
                <Button type="primary" :loading="trunCaseLoading" :disabled="trunCaseLoading" @click="examineSubmit('formValidate')">确认</Button>
        ...
        <Modal
            ...
            <div slot="header" style="font-weight:700;font-size:18px">
                <span>{{title}}</span>
                <Dropdown @on-click="listOption(conflictData.meList[0].clueId,$event)" class="edit-icon" v-if="conflictData">
                    <a href="javascript:void(0)">
                        <Icon type="ios-more" size="20"/>
                    </a>
                    <DropdownMenu slot="list">
                        <DropdownItem name="agree" v-if="conflictData.meList[0].state=='P'&&!conflictData.meList[0].isTestPurchase">通过</DropdownItem>
                        <DropdownItem name="firstAgree" v-if="conflictData.meList[0].state=='P'&&conflictData.meList[0].isTestPurchase">通过</DropdownItem>
                        <DropdownItem name="reject" v-if="conflictData.meList[0].state=='P'">驳回</DropdownItem>
                    </DropdownMenu>
                </Dropdown>
            </div>
            <compare-clue v-if="conflictCtrl" :defaultData="conflictData" :type="type"></compare-clue>
        </Modal>

         <!-- 线索初审通过-->
        <Modal
            v-model="firstPassCtrl"
            title="线索初审通过"
            :width="400"
            :footer-hide="true">
            <Row style="padding:10px 5px;font-size:14px;">是否初审通过该线索（需要测购的线索，需要测购通过才可以转案件）</Row>
            <Form ref="formData" :model="formData" :label-width="40">
                <FormItem label="备注" prop="passReamrk" 
                    :rules="[{ max:512,message:'不能超过512个字符',trigger:'blur'}]">
                    <Input v-model.trim="formData.passReamrk" type="textarea" :autosize="{minRows: 3,maxRows: 8}" placeholder='请填写备注信息(不超过512个字符)'></Input>
                </FormItem>
            </Form>
            <Row style="text-align:center;">
                <Button type="primary" :loading="trunCaseLoading" :disabled="trunCaseLoading" @click="firstPass([id],formData.passReamrk)">确认</Button>
                <Button @click="firstPassCtrl=false;" style="margin-left: 8px">取消</Button>
            </Row>
        </Modal>
    </div>
</template>
...
import testPurchaseForm from './plugin/testPurchaseForm.vue';
...
import compareClue from "./plugin/compareClue.vue";
...
export default {
    ...
    methods: {
        ...
        exportOption(name){
            switch (name) {
                case 'all':this.exportExcel('all');break;
                case 'part':this.exportExcel('part');break;
            }
        },
        headerOption(name){
        ...
        examineSubmit(){
            let url = '/business/case/batch-turn';
            let ajaxData = {
                clueIds:[],
            ...
            this.$refs['formValidate'].validate((valid)=>{
                if(valid){
                    if(this.rejectCtrl){
                        ...
                        ajaxData.caseType = this.formValidate.caseType;
                        ajaxData.correlationCaseIdList = this.formValidate.caseIdList?[this.formValidate.caseIdList]:[];
                        this.trunCaseLoading = true;
        ...
        firstPass(arr,remark){
            let url = "/business/clue/audit";
            let ajaxData = {
                clueIds:arr,
                remark:remark
            };
            if(this.firstPassCtrl){
                this.$refs['formData'].validate((valid)=>{
                    if(valid){
                        this.$patch(url,ajaxData).then(res=>{
                            this.firstPassCtrl = false;
                            if(res.code===0){
                                this.$Message.success('操作成功!');
                                this.reloadTable();
                            }else{
        ...
        //获取案件id
        getCaseId(data){
            this.formValidate.caseIdList = '';
            if(data){
                this.formValidate.caseIdList = data.id;
            }
        },
        getPushLink(){
        ...
        //查询冲突/关联
        getConflict(type){
            let url = type=="conflict"?"/business/clue/conflict":"/business/clue/correlation";
            let ajaxData = { id:this.id };
            this.$get(url,ajaxData).then(res=>{
                if(res.code===0){
        ...
        exportExcel(type){
            ...
            if(type==='part'){
                ajaxData.clueIds = this.idList.join(',')
            }else{
                delete ajaxData.clueIds;
            }
            delete ajaxData.type;
