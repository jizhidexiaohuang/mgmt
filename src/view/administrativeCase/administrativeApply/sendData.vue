<template>
    <div>
        <Button class="add-btn" type="primary" v-if="detail.sendMaterialList&&detail.sendMaterialList.length==0" @click="addCtrl=true">新建寄送</Button>
        <Row style="margin-top:10px;padding-left:10px;">
            <Row class="list" v-for="item in detail.sendMaterialList" :key="item.id">
            ...
            <Row class="list" v-for="item in detail.recipientMaterialList" :key="item.id">
    ...
    methods: {
        ...
        add(name){
            this.$refs[name].validate((vaild)=>{
                if(vaild){
                    let url = '/business/case/administrative/material-sent';
                    let ajaxData = {};
        ...
        getData(){
            this.addCtrl = false;
            let url = `/business/case/administrative/material-sent/${this.processId}`;
