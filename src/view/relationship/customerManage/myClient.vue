<template>
    <div class="customer-manage have-drawer-style relationship">
        <div class="card-style">
            ...
            <Row class="header-common-right-box">
                <Col style="float:left;">
                    <Button v-permission="['market-center:customer:my:manage-']" @click="addDrawerCtrl=true" type="primary">新增客户</Button>
                    <!-- <Button :disabled="idList.length==0" @click="transferCustomerCtrl=true">转移</Button>
                    <Button @click="exportExcel" type="primary" :loading="exportLoading" :disabled="exportLoading">导出</Button> -->
                    <Dropdown @on-click="headerOption">
                        ...
                        <DropdownMenu slot="list">
                            <DropdownItem v-permission="['market-center:customer:my:manage-']" 
                                        :disabled="idList.length==0" name="transfer">转移</DropdownItem>
                            <Dropdown placement="right-start" @on-click="exportOption" v-permission="['market-center:customer:all:export-']">
                ...
                <Col class="filter-col search-box">
                    <Input v-model="keyword" placeholder="客户名称" style="width: 250px;" 
                    ...
                    <div class="container" v-show="searchBoxCtrl">
                        ...
                        <Row class="header-common-right-box">
                            ...
                            <Col span="18">
                                <DatePicker v-model="cooperationDate" type="daterange" placement="bottom-end" placeholder="请选择时间" style="width:190px" @on-change="getTime"></DatePicker>
                            </Col>
                        </Row>
                        <Row class="header-common-right-box">
                            <Col span="6" class="header-common-col">
            ...
            <Row style="border:1px solid #dcdee2;border-bottom:0;">
                <checkbox-table :columns="columns" :data="customeList" ref="checkboxTable"
                    ...
                    <template slot="more-actions" slot-scope="{ row }">
                        <Dropdown @on-click="option(row,$event)" v-permission="['market-center:customer:my:manage-']">
        ...
        <Drawer 
            ...
            <div slot="header" class="drawer-title">新增客户</div>
            <Row class="detail-header">
                <span>基本信息</span>
            </Row>
            <editCustomer v-if="addDrawerCtrl" @closeDrawer="closeDrawer"></editCustomer>
        </Drawer>
        ...
        <Drawer 
            ...
            <div slot="header" class="drawer-title">
                ...
                <Dropdown transfer @on-click="option({id:id},'termination')" class="edit-icon"  
                    v-if="custCoopProgress&&custCoopProgress.indexOf('T')===-1" v-permission="['market-center:customer:my:manage-']">
                    <a href="javascript:void(0)">
                        <Icon type="ios-more" size="20"/>
                    </a>
                    <DropdownMenu slot="list">
            ...
            <cust-manage v-if="manageDrawerCtrl" :id="id" 
                :editFlag="isEditDetail" :followFlag="followFlag" @closeDrawer="closeDrawer"></cust-manage>
        </Drawer>
...
<script>
import editCustomer from "@/view/relationship/customerManage/customerForm";
...
export default {
    ...
    methods: {
        coopProgressState(data){
            let bool ;
            switch (true) {
                case (data=='P'||data=='I'): bool = false; break;//意向客户
                case (data=='R'||data=='S'||data=='E'||data=='T'): bool = true; break;//签约之后的
            }
            return bool;
        },
        headerOption(name){
            switch (name) {
                case 'transfer': 
                    this.transferCustomerCtrl = true;
                    break;
            }
        },
        exportOption(name){
    ...
    beforeMount(){
        this.getDictionary();
        this.userId = JSON.parse(window.localStorage.getItem('account')).uid;
    },
    mounted(){
        this.getData()
    },
