        ...
        <div class="card-style">
            ...
            <Row class="header-common-right-box">
                <Col style="float:left;">
                    <Button type="primary" v-permission="['market-center:opportunities:approval:manage-']" @click="openAddDrawer">立项申请</Button>
                </Col>
                <!-- 筛选项 -->
                        ...
                        <Row class="header-common-right-box">
                            ...
                            <Col span="18">
                                <Select v-model="searchForm.state" placeholder="请选择审核状态">
                                    <Option v-for="item in projectStatusArr" :value="item.value" :key="item.value">{{ item.nameZh }}</Option>
                                </Select>
                            </Col>
                        </Row>
                        <Row class="header-common-right-box">
            ...
            <Row style="border:1px solid #dcdee2;border-bottom:0;">
                ...
                <template slot-scope="{ row, index }" slot="action">
                    <Dropdown @on-click="option(row.id,$event)" v-permission="['market-center:opportunities:approval:manage-']">
                        <a href="javascript:void(0)">
                            <Icon type="ios-more" size="20"/>
                        </a>
                        <DropdownMenu slot="list">
                            <DropdownItem name="submit" v-show="row.state==='D'" v-permission="['market-center:opportunities:audit:audit-']">发布</DropdownItem>
                            <DropdownItem name="agree" v-show="row.state==='D'" v-permission="['market-center:opportunities:audit:audit-',false]">发布</DropdownItem>
                            <DropdownItem name="edit" v-show="row.state!=='E'">编辑</DropdownItem>
                            <DropdownItem v-show="row.state!=='T'" name="termination">终止</DropdownItem>
                        </DropdownMenu>
                    </Dropdown>
                </template>
                </Table>
            ...
            <Drawer 
                ...
                <div slot="header" class="drawer-title">
                    ...
                    <Dropdown transfer @on-click="option(projectDetail.id,$event)" class="edit-icon" 
                        v-show="isInfo" v-permission="['market-center:opportunities:approval:manage-']">
                        <a href="javascript:void(0)">
                            <Icon type="ios-more" size="20"/>
                        </a>
                        <DropdownMenu slot="list">
                            <DropdownItem name="submit" v-show="state==='D'" v-permission="['market-center:opportunities:audit:audit-']">发布</DropdownItem>
                            <DropdownItem name="agree" v-show="state==='D'" v-permission="['market-center:opportunities:audit:audit-',false]">发布</DropdownItem>
                            <DropdownItem name="edit" v-show="state!=='E'">编辑</DropdownItem>
                            <DropdownItem name="termination" v-show="state!=='T'">终止</DropdownItem>
                        </DropdownMenu>
                    </Dropdown>
                </div>
...
import projectApplicationInfo from './projectApplicationInfo.vue';
...
export default {
                ...
                },
                {
                    ...
                    render:(h,params)=>{
                        ...
                        switch (true){
                            ...
                            case state=='T':color = 'expireColor';break;
                        }
                        return h('Tag',{
                            class:'textover',
                            attrs:{
    ...
    methods: {
        ...
        agreeSubmit(){
            let url;
            if(this.type=='agree'){
                url = `/base/customer-opportunitie/${this.id}/audit`;
            }else if(this.type==='submit'){
                url = `/base/customer-opportunitie/${this.id}/submit-audit`;
            }
        ...
        //获取字典数据接口
        async getDictionary(){
            this.projectStatusArr = await this.common.getDictionaryItems('opportunities_state');
