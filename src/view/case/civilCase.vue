    ...
    <div class="have-drawer-style project-box">
        <div class="card-style">
            <ul class="case-tabs clearfix">
                    ...
                    <Poptip trigger="click" style="width:100%;" transfer placement="bottom-start">
                        ...
                        <div slot="content">
                            <CheckboxGroup @on-change="changePendding" v-model="penddingArr">
                                <Checkbox label="isPendingProgram" style="margin-bottom:10px;">诉讼方案待审核({{sumData.pendingProgramNumber || 0}})</Checkbox>
                                <Checkbox label="isPendingMaterial" style="margin-bottom:10px;">材料待寄送({{sumData.pendingMaterialNumber || 0}})</Checkbox>
                                <Checkbox label="isPendingLitigant" style="margin-bottom:10px;">当事人待审核({{sumData.pendingLitigantNumber || 0}})</Checkbox>
                                <Checkbox label="isPendingTermination" style="margin-bottom:10px;">终止待审核({{sumData.pendingTerminationNumber || 0}})</Checkbox><br/>
                                <Checkbox label="isPendingReconcile">和解待审核({{sumData.pendingReconcileNumber || 0}})</Checkbox>
                                <Checkbox label="isPendingSettlementPayment">收款待确认({{sumData.pendingSettlementPaymentNumber || 0}})</Checkbox>
                                <Checkbox label="isPendingMediation">调解待审核({{sumData.pendingMediationNumber || 0}})</Checkbox>
                                <Checkbox label="isPendingJointMediation">联调结果待审核({{sumData.pendingJointMediationNumber || 0}})</Checkbox>
                                <Checkbox label="isPendingWithdraw">撤诉待审核({{sumData.pendingWithdrawNumber || 0}})</Checkbox>
                            </CheckboxGroup>
                        </div>
                    </Poptip>
                </li>
                ...
                <li @click="changeTableColumns('CP')" :class="isSelected==='CP'?'active':''">已确定方案({{sumData.confirmProgramNumber || 0}})</li>
                <li @click="changeTableColumns('CF')" :class="isSelected==='CF'?'active':''">已立案({{sumData.confirmFilingNumber || 0}})</li>
                <li @click="changeTableColumns('S')" :class="isSelected==='S'?'active':''">二审({{sumData.secondInstanceNumber || 0}})</li>
...
export default {
    data () {
        return {
            ...
            penddingObj:{
                ...
                isPendingReconcile:0,
                isPendingSettlementPayment:0,
                isPendingMediation:0,
                isPendingJointMediation:0,
                isPendingWithdraw:0,
            }
        }
    },
    ...
    methods: {
        ...
        //获取列表数据
        getData(){
            let url = '/business/case/my/civil';
            let ajaxData = {
