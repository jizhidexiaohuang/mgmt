    ...
    <div class="mediate">
        ...
        <Row class="case-content">
            ...
            <Row class="detail-content" v-if="isDetailLoad&&defaultData">
                ...
                <Row><Col span="5" class="left-label">调解金额：</Col><Col span="16" class="right-content">{{common.formateNumber(defaultData.fee || "")}}</Col></Row>
                ...
                <Row><Col span="5" class="left-label">审核备注：</Col><Col span="16" class="right-content">{{defaultData.auditRemark}}</Col></Row>
                <Row v-if="defaultData.result">
                    <Col span="5" class="left-label">调解结果：</Col>
                    <Col span="16" class="right-content">{{defaultData.result.succeed?"成功":"失败"}}</Col>
                </Row>
                <Row v-if="defaultData.result">
                    <Col span="5" class="left-label">调解协议：</Col>
                    <Col span="16" class="right-content">
                        <file-upload
                            :defaultList="defaultData.result.attachments || []" 
        ...
        <Modal
            ...
            :width="360"
            :footer-hide="true">
            <Row class="modal-tips">请问是否审核{{auditType=="agree"?"通过":"驳回"}}这份调解方案？</Row>
            <Form ref="formData" :model="formData" :label-width="80">
                <FormItem label="审核备注" prop="remark" 
                    ...
                    <Input v-model.trim="formData.remark" type="textarea" :autosize="{minRows: 3,maxRows: 8}" placeholder='请填写审核备注(不超过512个字符)'></Input>
                </FormItem>
            </Form>
...
export default {
    ...
    methods: {
        //获取调解详情
        getDetail(){
            let url = '/business/case/mediation?type=ME&processId='+this.processId;
        ...
        agreeSubmit(){
            let url = '';
            let ajaxData = { remark:this.formData.remark };
            if(this.auditType=='agree'){
                url = `/business/case/mediation/${this.defaultData.id}/audit`;
                this.isSubmit = true;
                ...
                url = `/business/case/mediation/${this.defaultData.id}/reject`;
