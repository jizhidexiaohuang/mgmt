<template>
    <div>
        <Button class="add-btn" type="primary" v-if="detail.sendCaseMaterialSentList&&detail.sendCaseMaterialSentList.length==0" @click="addCtrl=true">新建寄送</Button>
        <Row 
            v-if="(detail.sendCaseMaterialSentList&&detail.sendCaseMaterialSentList.length!=0) || (detail.recipientCaseMaterialSentList&&detail.recipientCaseMaterialSentList.length!=0)" 
            style="margin-top:10px;padding-left:10px;">
            <Row class="list" v-for="item in detail.sendCaseMaterialSentList" :key="item.id">
                ...
                <Col>
                    <span class="body">{{item.sentTimeStr}}</span><br/>
                    <span class="header">寄件时间</span>
                </Col>
                <Col>
                    <span class="body">{{item.company}}</span><br/>
                ...
                <Col>
                    <span class="body">
                        <Tooltip max-width="200" :content="getContentStr(item.materialSentContentList)">
                            <div class="textover" style="width:200px;">{{getContentStr(item.materialSentContentList)}}</div>
                        </Tooltip>
                    </span><br/>
            ...
            <Row class="list" v-for="item in detail.recipientCaseMaterialSentList" :key="item.id">
        ...
        <Modal
            ...
            <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="80">
                ...
                <FormItem 
                    v-for="(item,index) in formValidate.materialSentContentList" 
                    :label="index==0?'邮寄内容':''"
                    :prop="'materialSentContentList.' + index"
    ...
    methods: {
        ...
        add(name){
            this.$refs[name].validate((vaild)=>{
                if(vaild){
                    let url = '/business/case/program/material-sent';
                    let ajaxData = {};
        ...
        getData(){
            this.addCtrl = false;
            let url = `/business/case/program/material-sent/${this.processId}`;
        ...
        edit(item){
            ...
            for(let key in this.formValidate){
                if(key==='materialSentContentList'){
                    this.formValidate.materialSentContentList = data.materialSentContentList.map(item=>{
                        return {
                            name:item.name,
                            num:item.num
                        }
            ...
            this.formValidate.sentTime = data.sentTimeStr;
            this.addCtrl = true;
        },
        //获取物流信息
        ...
        addFile(){
            this.formValidate.materialSentContentList.push({
                name:'',
                num:1
            })
        },
        removeFile(index){
            this.formValidate.materialSentContentList.splice(index,1);
        },
        //获取字典数据接口
        async getDictionary(){
