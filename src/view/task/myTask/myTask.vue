    ...
    <div class="have-drawer-style">
        <div class="card-style">
                    ...
                    <div class="container" v-show="searchBoxCtrl">
                        <Row class="header-common-right-box">
                            ...
                            <Col span="18" style="line-height:32px;">
                                <span :class="['expiry-date-option',searchForm.timeScope==3&&'active']" @click="searchForm.timeScope=3">三天内</span>
                                <span :class="['expiry-date-option',searchForm.timeScope==7&&'active']" @click="searchForm.timeScope=7">一周内</span>
                                <span :class="['expiry-date-option',searchForm.timeScope==15&&'active']" @click="searchForm.timeScope=15">15天内</span>
                            </Col>
                        </Row>
                        <Row class="header-common-right-box">
                            <Col span="18" offset="6">
                                <DatePicker type="daterange" v-model="deadline"
                                    @on-change="getExpiryDate" placement="bottom-end" placeholder="请选择时间" style="width:190px"></DatePicker>
                            </Col>
                        </Row>
                        <Row class="header-common-right-box">
                            <Col span="6" class="header-common-col">
                            ...
                            <Col span="18">
                                <City ref="cityCascader" v-on:listenCity="getCity" style="width:190px;"></City>
                            </Col>
                        </Row>
                        <Row class="header-common-right-box">
                            <Col span="6" class="header-common-col">
                        ...
                        <Row class="header-common-right-box">
                            ...
                            <Col span="18">
                                <Select v-model="searchForm.missionType" placeholder="请选择任务类型">
                                    <Option v-for="item in missionTypeArr" :value="item.value" :key="item.value">{{ item.nameZh }}</Option>
                                </Select>
                            </Col>
                        </Row>
                        <Row class="header-common-right-box">
                            ...
                            <Col span="18">
                                <Select v-model="searchForm.state" placeholder="请选择任务状态">
                                    <Option v-for="item in missionStateArr" :value="item.value" :key="item.value">{{ item.nameZh }}</Option>
                                </Select>
                            </Col>
                        </Row>
            ...
            <Row style="border:1px solid #dcdee2;border-bottom:0;">
                ...
                <template slot-scope="{ row, index }" slot="action">
                    <Dropdown @on-click="option(row,$event)" v-permission="['mission:my:manage-']">
                        <a href="javascript:void(0)">
                            <Icon type="ios-more" size="20"/>
                        </a>
                        <DropdownMenu slot="list">
                            <DropdownItem name="edit">编辑</DropdownItem>
                            <DropdownItem name="rejectReason" v-show="row.state=='RT'">查看驳回原因</DropdownItem>
                            <DropdownItem name="delete" v-show="getMissionState(row.state,'delete')">删除</DropdownItem>
                            <DropdownItem name="publish" v-permission="['mission:audit-']" v-show="getMissionState(row.state,'publish')">发布</DropdownItem>
                            <DropdownItem name="publish-P" v-permission="['mission:audit-',false]" v-show="getMissionState(row.state,'publish-P')">发布</DropdownItem>
                            <DropdownItem name="close" v-show="getMissionState(row.state,'close')">关闭</DropdownItem>
                            <DropdownItem name="open" v-show="getMissionState(row.state,'open')">开启</DropdownItem>
                            <DropdownItem name="delay" v-show="getMissionState(row.state,'delay')">延期</DropdownItem>
                        </DropdownMenu>
                    </Dropdown>
                </template>
                </Table>
        ...
        <Drawer 
            ...
            <div slot="header" class="drawer-title">
                ...
                <Dropdown transfer @on-click="option(taskDetail,$event)" class="edit-icon" v-if="isInfo" v-permission="['mission:my:manage-']">
                    <a href="javascript:void(0)">
                        <Icon type="ios-more" size="20"/>
                    </a>
                    <DropdownMenu slot="list">
                        <DropdownItem name="edit">编辑</DropdownItem>
                        <DropdownItem name="rejectReason" v-show="taskDetail.state=='RT'">查看驳回原因</DropdownItem>
                        <DropdownItem name="delete" v-show="getMissionState(taskDetail.state,'delete')">删除</DropdownItem>
                        <DropdownItem name="publish" v-permission="['mission:audit-']" v-show="getMissionState(taskDetail.state,'publish')">发布</DropdownItem>
                        <DropdownItem name="publish-P" v-permission="['mission:audit-',false]" v-show="getMissionState(taskDetail.state,'publish-P')">发布</DropdownItem>
                        <DropdownItem name="close" v-show="getMissionState(taskDetail.state,'close')">关闭</DropdownItem>
                        <DropdownItem name="open" v-show="getMissionState(taskDetail.state,'open')">开启</DropdownItem>
                        <DropdownItem name="delay" v-show="getMissionState(taskDetail.state,'delay')">延期</DropdownItem>
                    </DropdownMenu>
                </Dropdown>
            </div>
        ...
        <Modal
            ...
            <Form ref="formValidate" :model="formValidate">
                ...
                <FormItem label="截止日期" prop="date" v-if="delayCtrl" style="width:100%" 
                    :rules="[{required: true, message: '请选择截止日期', trigger: 'blur'}]">
                    <DatePicker type="date" :options="dateOption" @on-change="getDate" placement="bottom-end" placeholder="请选择截止日期"></DatePicker>
                </FormItem>
...
export default {
    ...
    props: [''],
    methods: {
        // DT:草稿 || BR:待审核 || RD:进行中 || RT:驳回修改 || PE:关闭 || OT:过期
        getMissionState(state,name){
            switch (name) {
                case 'publish':
                    ...
                    break;
                case 'publish-P':
                    return ['DT'].indexOf(state)!=-1?true:false;
                    break;
                case 'delete':
                    return state==='DT'?true:false;
                    break;
                case 'reject':
        ...
        option(row,name){
            ...
            switch (true) {
                case (name==='edit'): 
                    this.isInfo = false;
                    this.isEdit = true;
                    this.getTaskDetail(this.id,'edit');
                    break;
                case (name==='delete'||name==='open'||name==='publish'||name==='publish-P'):
                ...
                case (name==='reject'||name==='close'||name==='delay'):
                    ...
                    break;
                case (name==='rejectReason'):
                    this.rejectReason = row.auditRemark;
                    this.rejectReasonCtrl = true;
                    break;
            }
        },
        agreeSubmit(){
            ...
            }else if(this.type==='publish'||this.type==='publish-P'||this.type==='open'){
                url = this.type==='publish'?'/business/mission/audit':(this.type==='publish-P'?'/business/mission/submit':'/business/mission/open');
        ...
        getExpiryDate(data){
            console.log(data)
            this.searchForm.beginDate = data[0];
        ...
        async getDictionary(){
            ...
            this.missionStateArr = await this.common.getDictionaryItems('mission_state');
        },
    },
    created(){},
    activated(){
